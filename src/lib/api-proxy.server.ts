...
const REQUEST_HEADERS = ['accept', 'content-type', 'cookie', 'origin', 'user-agent', 'sec-fetch-site'];
...
function json(status: number, code: string): Response {
  return new Response(JSON.stringify({ error: { code, message: code } }), {
    status, headers: { ...SAFETY_HEADERS, 'content-type': 'application/json' },
  });
}
...
export async function proxyApi(request: Request, backend: string | undefined,
  ...
  try {
    ...
    const body = ['GET', 'HEAD'].includes(request.method) ? undefined : await readBody(request, signal);
    const upstream = await fetcher(`${origin}${incoming.pathname}${incoming.search}`, {
      method: request.method, headers, body, redirect: 'manual', signal,
    });
