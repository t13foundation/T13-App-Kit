...
export function readSupabaseConfig(env: Record<string, unknown>): PublicSupabaseConfig | null {
  ...
  let parsed: URL;
  try { parsed = new URL(url); } catch { throw new Error("The Supabase origin is invalid."); }
  const local = ["localhost", "127.0.0.1", "[::1]"].includes(parsed.hostname);
  if ((parsed.protocol !== "https:" && !(local && parsed.protocol === "http:")) ||
      parsed.username || parsed.password || parsed.search || parsed.hash || parsed.pathname !== "/") {
    throw new Error("Use an HTTPS Supabase origin, or an HTTP loopback origin for local development.");
  }
  if (!/^sb_publishable_[A-Za-z0-9_-]+$/.test(key)) {
    ...
    try {
      ...
      if (!claims || typeof claims !== "object" || !("role" in claims) || claims.role !== "anon") throw new Error();
    } catch {
