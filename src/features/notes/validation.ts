...
export function validateNoteDraft(value: NoteDraft): NoteDraft {
  ...
  if (!title || title.length > NOTE_TITLE_LIMIT) throw new Error("Tytuł musi mieć od 1 do 120 znaków.");
  if (value.body.length > NOTE_BODY_LIMIT) throw new Error("Treść może mieć maksymalnie 20 000 znaków.");
