...
export async function listNotes(
  ...
  const { data, error } = await safeQuery(
    client
      ...
      .range(page * NOTES_PAGE_SIZE, (page + 1) * NOTES_PAGE_SIZE)
      .abortSignal(signal),
...
export async function saveNote(
  ...
  const result = original
    ? await safeQuery(
        client
          ...
          .select(columns)
          .maybeSingle(),
        "Nie potwierdzono zapisu. Odśwież listę przed ponowną próbą.",
      )
    : await safeQuery(
        client
          .from("notes")
          .insert({ ...payload, owner_id: actorId })
          .select(columns)
          .single(),
        "Nie potwierdzono zapisu. Odśwież listę przed ponowną próbą.",
      );
  ...
  if (!result.data)
    throw new Error("Notatka została zmieniona albo nie jest już dostępna. Odśwież listę.");
  return result.data;
}
...
export async function deleteNote(
  ...
  const { data, error } = await safeQuery(
    client
      ...
      .select("id")
      .maybeSingle(),
    "Nie potwierdzono usunięcia. Odśwież listę przed ponowną próbą.",
  );
