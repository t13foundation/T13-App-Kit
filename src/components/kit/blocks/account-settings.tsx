...
import { preferencesSchema, profileSchema } from '@shared/contracts';
...
/** Real account operations. Credentials and recovery material stay in memory only. */
export function AccountSettings({ me, refresh }: { me: Me; refresh: () => Promise<unknown> }) {
  ...
  async function run(id: string, operation: () => Promise<unknown>, success: string) {
    ...
    try { await operation(); if (success) setNotice({ error: false, message: success }); }
      ...
      setNotice({ error: true, message: cause instanceof ApiRequestError ? cause.message : 'Nie udało się wykonać operacji.',
        reauthenticate: cause instanceof ApiRequestError && ['reauthentication_required', 'unauthenticated'].includes(cause.code) });
  ...
  return <div className="mx-auto w-full max-w-4xl px-5 py-10 sm:px-8">
    <div className="mb-8 flex flex-wrap items-start justify-between gap-4">
      ...
      <Button type="button" color="secondary" isDisabled={disabled} onClick={() => void run('logout', async () => {
        await signOut(); queryClient.clear(); window.location.assign('/sign-in');
      }, '')}>Wyloguj się</Button>
    </div>
    ...
    <Section title="Preferencje" description="Ustawienia zapisywane na Twoim koncie.">
      <form className="space-y-4" onSubmit={(event) => {
        ...
        if (!parsed.success) return invalid('Wpisz język pl lub en i poprawną strefę czasową, np. Europe/Warsaw.');
        void run('preferences', async () => { await updatePreferences(parsed.data); await refresh(); }, 'Preferencje zostały zapisane.');
      }}>
        <Input name="locale" label="Język i formaty regionalne (pl / en)" defaultValue={me.locale} isRequired maxLength={2} />
        <Input name="timezone" label="Strefa czasowa" defaultValue={me.timezone} isRequired hint="Np. Europe/Warsaw, Europe/London lub UTC." />
        <p className="text-xs text-gray-600">W tym wydaniu interfejs jest po polsku. Ustawienie języka zmienia format dat; pełne tłumaczenia nie są jeszcze dostępne.</p>
    ...
    <Section title="Logowanie dwuetapowe" description="Kod z aplikacji uwierzytelniającej i jednorazowe kody odzyskiwania.">
      ...
      {enrollment ? <div className="space-y-4">
        ...
        {!confirmed ? <form className="space-y-4" onSubmit={(event) => {
    ...
    <Section title="Dane konta" description="Pobierz profil i preferencje. Ten eksport nie obejmuje jeszcze materiałów innych modułów.">
      <Button type="button" color="secondary" isDisabled={disabled} isLoading={busy === 'export'} onClick={() => void run('export', async () => {
        ...
        const link = document.createElement('a'); link.href = url; link.download = 'konto.json'; link.click();
