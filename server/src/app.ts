...
async function requireSession(request: FastifyRequest, reply: FastifyReply, fresh = false) {
  ...
  if (!session.user.emailVerified) {
    fail(reply, 403, "email_not_verified");
    return null;
  }
  if (fresh && !isFreshSession(session.session.createdAt)) {
    fail(reply, 403, "reauthentication_required");
    return null;
  }
  return session;
}
...
export async function buildApp(): Promise<FastifyInstance> {
  ...
  app.addHook("preHandler", async (request, reply) => {
    ...
    if (problem)
      return fail(
        reply,
        problem === "unsupported_content_type" ? 415 : 403,
  ...
  app.get("/api/status", async (_request, reply) => {
    ...
    try {
      ...
      /* no dependency details in responses */
    }
  ...
  app.route({
    ...
    handler: async (request, reply) => {
      ...
      if (body && typeof body === "object" && !Array.isArray(body)) {
        ...
        if (path === "/two-factor/enable" && data.method && data.method !== "totp")
          return fail(reply, 400, "unsupported_second_factor");
        if (path === "/change-password") body = { ...data, revokeOtherSessions: true };
      }
      const response = await auth().handler(
        new Request(new URL(request.url, cfg.AUTH_URL), {
          method: request.method,
          headers: authHeaders(request),
      ...
      for (const key of ["content-type", "location", "retry-after"]) {
        ...
        if (value !== null) reply.header(key, value);
      }
  ...
  app.post("/api/me/export", async (request, reply) => {
    const session = await requireSession(request, reply, true);
    if (!session) return;
    return {
      schemaVersion: 1,
  ...
  app.setErrorHandler((error: unknown, request, reply) => {
    ...
    return fail(
      reply,
      status,
      status === 429 ? "rate_limited" : status === 413 ? "payload_too_large" : "request_failed",
    );
  });
