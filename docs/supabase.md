...
This is the technical contract for WBS-APP1-01. The accepted target is TanStack Start + React + Supabase + Cloudflare. `/notes` contains the new signup, six-digit email confirmation, login, local logout and Notes flow. The existing account routes and `server/` remain the isolated legacy implementation until replacement acceptance. The shell links to Notes; legacy account routes remain separate and their existing guides are not instructions for the Supabase path.
...
docker context ls
colima list
# Only create this profile when absent; do not resize an existing shared profile.
...
The configured project ID is `t13-app-kit-demo`, with API 55321, database 55322, shadow 55320, optional Studio 55323 and Mailpit 55324. Before any synthetic signup or mutation, inspect its public table names and migration history: only this kit’s `notes` table and `20260910002051_private_notes` migration should be present. The unrelated existing `t13-app-kit-local` project must remain untouched.
...
`migration up --local` applies only pending versioned migrations and preserves existing local data. Do not run `db reset` against the healthy shared test instance.
...
Run the web app with `pnpm dev --host 127.0.0.1 --port 4311 --strictPort`, then open `/notes`. The Vite/TanStack plugin must generate the actual route tree; do not hand-edit `src/routeTree.gen.ts`. Local email confirmations are enabled, tokens expire after 600 seconds, and the custom template contains a code rather than an authentication URL. Mailpit is configured on port 55324. The lead configures only signup confirmation mail for the dedicated hosted test demo. General email and Turnstile rollout remain outside this slice.
...
## Dedicated hosted test demo

The lead owns provider configuration and deployment. A dedicated Supabase test project needs the existing versioned migration, email confirmations enabled, 12-character minimum passwords, 600-second OTP/JWT lifetime and the confirmation template from `supabase/templates/confirmation.html`. Hosted signup mail must use an authorized SMTP sender; default hosted mail is not assumed to deliver to arbitrary test recipients. Anonymous sign-ins remain disabled.

Build the same commit for the dedicated Cloudflare test target with `VITE_SUPABASE_URL` and `VITE_SUPABASE_PUBLISHABLE_KEY` set to that test project’s public values. They are build-time browser configuration, not server secrets. No service-role key is needed by the app. Preserve the current Nitro Cloudflare target and use its generated Wrangler configuration after `pnpm build`; the lead checks provider authentication and the exact deployment command. Configure the dedicated app origin in the test project’s Auth settings. Do not reuse customer projects or deploy the isolated legacy backend.
