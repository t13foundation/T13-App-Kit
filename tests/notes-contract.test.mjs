...
const env = (url, key = publicKey) => ({ VITE_SUPABASE_URL: url, VITE_SUPABASE_PUBLISHABLE_KEY: key });
...
test('accepts only HTTPS or HTTP loopback origins', () => {
  ...
  for (const url of ['http://example.test', 'https://user:pass@example.test', 'https://example.test/path', 'https://example.test?token=x', 'https://example.test#token', 'file:///tmp']) {
...
test('configuration errors never echo supplied keys or malformed URLs', () => {
  ...
  assert.throws(() => readSupabaseConfig(env(unsafe)), (error) => !error.message.includes(unsafe));
  assert.throws(() => readSupabaseConfig(env('https://example.test', unsafe)), (error) => !error.message.includes(unsafe));
});
test('rejects service-role and secret keys but permits legacy local anon JWT shape', () => {
  ...
  for (const key of [jwt('service_role'), jwt('authenticated'), 'sb_secret_synthetic', 'not-a-jwt']) {
...
test('note input trims title, preserves body, and enforces bounds', () => {
  ...
  for (const title of ['', ' \n\t', 'x'.repeat(121)]) assert.throws(() => validateNoteDraft({ title, body: '' }));
  assert.throws(() => validateNoteDraft({ title: 'Valid', body: 'x'.repeat(20001) }));
  assert.equal(validateNoteDraft({ title: 'x'.repeat(120), body: 'x'.repeat(20000) }).body.length, 20000);
});
test('private Notes response strips public caching without touching unrelated responses', async () => {
  for (const path of ['/notes', '/notes/']) {
    const original = new Response('shell', { headers: { 'cache-control': 'public,max-age=3600', 'set-cookie': 'synthetic=value' } });
  ...
  assert.equal(protectNotesResponse(new Request('https://example.test/catalog'), original), original);
});
...
function fakeClient(result) {
  ...
  for (const method of ['single', 'maybeSingle', 'abortSignal']) {
    chain[method] = (...args) => { calls.push([method, ...args]); return Promise.resolve(result); };
  }
  return { client: chain, calls };
}
const original = { id: 'note-a', title: 'Before', body: '', created_at: '2026-01-01', updated_at: '2026-01-02' };
test('listing binds actor, uses stable ordering and requests one look-ahead row', async () => {
  const { client, calls } = fakeClient({ data: Array.from({ length: 21 }, (_, id) => ({ ...original, id: String(id) })), error: null });
  const signal = new AbortController().signal;
  const result = await listNotes(client, 'actor-a', 2, signal);
  assert.equal(result.notes.length, 20); assert.equal(result.hasMore, true);
  assert.ok(calls.some(([method, key, value]) => method === 'eq' && key === 'owner_id' && value === 'actor-a'));
  assert.deepEqual(calls.find(([method]) => method === 'range'), ['range', 40, 60]);
...
test('insert binds the originating identity, not whichever session wins a later race', async () => {
  ...
  assert.deepEqual(calls.find(([method]) => method === 'insert'), ['insert', { title: 'New', body: 'Body', owner_id: 'actor-a' }]);
});
test('update and delete require actor, identity and original timestamp', async () => {
  for (const operation of ['update', 'delete']) {
    const { client, calls } = fakeClient({ data: original, error: null });
    if (operation === 'update') await saveNote(client, 'actor-a', { title: 'After', body: '' }, original);
    else await deleteNote(client, 'actor-a', original);
    assert.deepEqual(calls.filter(([method]) => method === 'eq'), [['eq', 'owner_id', 'actor-a'], ['eq', 'id', original.id], ['eq', 'updated_at', original.updated_at]]);
  }
});
test('zero-row and unknown outcomes are not reported as successful mutations', async () => {
  ...
  await assert.rejects(saveNote(missing, 'actor-a', { title: 'After', body: '' }, original), /zmieniona/);
  ...
  await assert.rejects(saveNote(failed, 'actor-a', { title: 'After', body: '' }), (error) => !error.message.includes('SENSITIVE_PROVIDER_ERROR'));
