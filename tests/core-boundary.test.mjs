...
const request = (path = "/api/me/preferences", extra = {}) =>
...
test("mutations require a trusted origin and reject simple form submissions", () => {
  assert.equal(
    mutationProblem("PUT", undefined, "application/json", [publicOrigin]),
    "origin_not_allowed",
  );
  assert.equal(
    mutationProblem("POST", publicOrigin, "text/plain", [publicOrigin]),
    "unsupported_content_type",
  );
  assert.equal(
    mutationProblem("POST", publicOrigin, "application/json; charset=utf-8", [publicOrigin]),
    null,
  );
});
...
test("cross-origin writes and oversized bodies are rejected before upstream access", async () => {
  ...
  const oversized = await proxyApi(
    request("/api/me/preferences", { body: "x".repeat(MAX_API_BODY_BYTES + 1) }),
    backend,
    fetcher,
  );
...
test("network failures return a safe error without the upstream exception contents", async () => {
  const result = await proxyApi(request(), backend, async () => {
    throw new Error("secret-and-private-url");
  });
